...
- **Leaner OpenAPI / Swagger docs** (0.4.3) - the API server now collapses FastAPI's duplicate `Foo-Input`/`Foo-Output` schemas into a single `Foo` (new `api.openapi.separate_schemas`, default `False`), and the new `api.openapi.docs.models_expansion` (default `-1`) hides Swagger's standalone Schemas section.  Together they roughly halve `openapi.json` size and keep the docs responsive for apps with large model graphs.  See the [0.4 Changelog](changelog/0.4.md#044).
